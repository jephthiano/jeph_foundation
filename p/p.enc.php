...
if(isset($_GET['title'])){// getting the value in $_GET  and assigning the value of id
	...
	$id = content_data('programme_table','p_id',($raw_title),'p_title');
	if($id === false){trigger_error_manual();}
	if(content_data('programme_table','p_status',($raw_title),'p_title') === 'pending'){trigger_error_manual();}
}else{
	trigger_error_manual();
}
//meta
$follow_type = 'follow';
$image_link = file_location('media_url',get_media('programme',$id));
$image_type = substr($image_link,-3);
$page = ucwords(($raw_title))." | ".strtoupper(get_xml_data('company_name'));
$page_name = $page." | ".get_xml_data('seo_tag');
$page_url = file_location('home_url','p/p/'.urlencode(($raw_title)));
