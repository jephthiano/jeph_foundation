...
$page_url = file_location('admin_url','programme/update_programme/'.$_GET['page']);
$page = "UPDATE PROGRAMME";
			...
			<center>
				...
				<div class="j-container">
					...
					if($id === false){
						...
						?>
						<h2 class='j-text-color1 j-center'><b>UPDATE PROGRAMME</b></h2>
						<div>
							<div id='preview'class='j-border-color5 j-border j-circle j-color5 j-vertical-center-container'style='width:100px;height:100px;'
								 onclick="$('#programme_pics_modal').show();">
								<img src='<?=file_location('media_url',get_media('programme',$id))?>'alt=''class='j-circle j-card-4'style='width:100px;height:100px;border:solid 2px white'>
								<span class='j-text-color4 j-bold j-vertical-center-element'style='font-size:40px;'>+</span>
							</div>
							<br><br>
							<?php image_modal('programme',$id)?>
						</div>
						<form onsubmit="event.preventDefault();"class=''>
							...
							<input type="text"class="j-input j-medium j-border-2 j-border-color5 j-round-large"placeholder="Title"
								name="tt"id="tt"value="<?=content_data('programme_table','p_title',$cid,'p_id')?>"style="width:100%;max-width:400px"/>
							<br>
							...
							<input type="text"class="j-input j-medium j-border-2 j-border-color5 j-round-large"placeholder="Keywords"
								name="ky"id="ky"value="<?=content_data('programme_table','p_keyword',$cid,'p_id')?>"style="width:100%;max-width:400px"/>
							...
							><?=content_data('programme_table','p_paragraph1',$cid,'p_id')?></textarea>
							<br>
							...
							><?=content_data('programme_table','p_paragraph2',$cid,'p_id')?></textarea>
							...
							><?=content_data('programme_table','p_paragraph3',$cid,'p_id')?></textarea>
							<div class='j-small j-itallic'>(optional)</div>
							<br>
							...
							<input type="text"class="j-input j-medium j-border-2 j-border-color5 j-round-large"placeholder="Datetime"
								name="dt"id="dt"value="<?=content_data('programme_table','p_regdatetime',$cid,'p_id')?>"style="width:100%;max-width:400px"/>
							...
							<button type='submit'id='sbtn'class="j-btn j-medium j-color1 j-round-large j-bolder">Update Programme</button>
						</form>
						<?php
					}
					?>
