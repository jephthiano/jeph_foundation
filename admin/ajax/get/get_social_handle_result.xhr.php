...
if(isset($_POST['s'])){
	...
	if(!empty($searchtext)){ // if the search text is not empty
		...
		$sql = "SELECT s_id,s_name,s_icon,s_link FROM social_handle_table ORDER BY s_id DESC";
		$stmt = $conn->prepare($sql);
		$stmt->bindColumn('s_id',$id);
		$stmt->bindColumn('s_name',$name);
		$stmt->bindColumn('s_icon',$icon);
				...
				<table class="j-table-all j-border-0 j-large">
					...
					while($stmt->fetch()){
						...
						<tr class="j-border-0">
							...
							<td><a href='<?= file_location('admin_url','social_handle/preview_social_handle/'.addnum($id))?>'><i class="<?= icon('eye');?>"></i></a></td>
							<td><a href='<?= file_location('admin_url','social_handle/update_social_handle/'.addnum($id))?>'><i class="<?= icon('edit');?>"></i></a></td>
