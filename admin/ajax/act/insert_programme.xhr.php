...
if(isset($_POST["tt"]) && isset($_POST["ky"]) && isset($_POST["p1"]) && isset($_POST["p2"]) && isset($_POST["p3"]) && isset($_POST["dt"])){
	...
	if(empty($error) and empty($missing)){
		//FORM IMAGE UPLOAD
		$location = 'programme';$size = 50000000;$file_mode = ["image/png","image/jpeg"];$file_type = 'image';
		...
		if(empty($missing) && empty($error)){
			if($file2 === "larger"){ // if file is larger tha expected echo error
				...
				$programme->type = $file2;
				if($file2 === "normal"){$programme->file_name = $correct['filename'];$programme->extension = $correct['extension'];}
				$insert = $programme->insert_programme();
				if($insert == true && is_numeric($insert)){
					$data["status"] = 'success';$data["message"] = file_location('admin_url','programme/preview_programme/'.addnum($insert));
				}elseif($insert === 'exists'){
					$data["status"] = 'fail';$data["message"] = 'Programme already exists';
				}elseif($insert === false){
